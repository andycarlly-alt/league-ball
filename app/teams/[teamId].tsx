  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#061A2B", padding: 16 }}>
      ...
      <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
        ...
        >
          <Text style={{ fontWeight: "900", color: "#061A2B" }}>{transferMode ? "Transfer: ON" : "Transfer"}</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView style={{ marginTop: 14 }} contentContainerStyle={{ gap: 12, paddingBottom: 30 }}>
        {roster.map((p) => {
          ...
          return (
            <View key={p.id} style={{ backgroundColor: "#0A2238", borderRadius: 14, overflow: "hidden", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)" }}>
              <View style={{ backgroundColor: banner.bg, paddingVertical: 8, paddingHorizontal: 12, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                <Text style={{ color: banner.fg, fontWeight: "900" }}>{banner.label}  Age {age}</Text>
                <Text style={{ color: banner.fg, fontWeight: "900" }}>{p.verified ? " VERIFIED" : " UNVERIFIED"}</Text>
              </View>

              <View style={{ padding: 14 }}>
                <Text style={{ color: "#EAF2FF", fontWeight: "900", fontSize: 16 }}>{p.fullName}</Text>
                <Text style={{ color: "#9FB3C8", marginTop: 6 }}>DOB: {p.dob}</Text>
                {p.verificationNote ? <Text style={{ color: "#22C6D2", marginTop: 6, fontWeight: "700" }}>{p.verificationNote}</Text> : null}
                ...
                <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
                  <TouchableOpacity
                    ...
                    style={{ flex: 1, backgroundColor: p.verified ? "#FF3B30" : "#34C759", padding: 10, borderRadius: 12, alignItems: "center", opacity: canVerify ? 1 : 0.5 }}
                    disabled={!canVerify}
                  >
                    <Text style={{ fontWeight: "900", color: "#061A2B" }}>{p.verified ? "Unverify" : "Verify"}</Text>
                  </TouchableOpacity>
                  ...
                  <TouchableOpacity
                    ...
                    style={{ backgroundColor: "#F2D100", padding: 10, borderRadius: 12, alignItems: "center", opacity: transferMode && canTransfer ? 1 : 0.4 }}
                ...
                {!isPro ? (
                  <Text style={{ color: "#9FB3C8", marginTop: 10 }}>
                    Free plan note: roster lock/transfers/admin audit tools are disabled until Pro.
                  </Text>
                ) : null}
              </View>
            </View>
        ...
        {!roster.length ? <Text style={{ color: "#9FB3C8" }}>No players yet.</Text> : null}
      </ScrollView>
    </View>
  );
}
